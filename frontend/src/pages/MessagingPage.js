...
const MessagingPage = () => {
    ...
    const handleComposeMessage = async () => {
        try {
            const messageData = {
                receiver: { id: parseInt(composeData.receiverId) },
                sender: { id: currentUser?.id || 1 },
    ...
    const getUnreadCount = (userId) => {
        return messages.filter(msg => 
            (msg.receiver?.id === currentUser?.id || msg.receiverId === currentUser?.id) && 
            (msg.sender?.id === userId || msg.senderId === userId) && 
            !msg.isRead
        ).length;
    };
                            ...
                            <>
                                ...
                                <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
                                    {conversation.map((message) => {
                                        const isOwnMessage = (message.sender?.id || message.senderId) === currentUser?.id;
                                        return (
                                            <Box
                                                key={message.id}
                                                sx={{
